import {
    active,
    addClass,
    bootstrapCustomEvent, bottom,
    clickEvent, clientHeight, dataInterval, dataKeyboard, dataPause, dataRide, DOC,
    emulateTransitionEnd,
    getAttribute, getBoundingClientRect,
    getElementsByClassName, getElementsByTagName, globalObject,
    hasClass, hoverEvent, HTML, indexOf, interval, keyboard, keydownEvent,
    left,
    length, mouseHover, off,
    offsetWidth,
    on, passiveHandler, pause,
    preventDefault,
    push,
    queryElement,
    removeClass, right, slideEvent, slidEvent, stringCarousel,
    supports,
...
const Carousel = function (element, options) {
    ...
    // DATA API
    const intervalAttribute = element[getAttribute](dataInterval),
    ...
    // bind, event targets
    const self = this;
    let index = element.index = 0, timer = element.timer = 0,
        isSliding = false, // isSliding prevents click event handlers when animation is running
        isTouch = false, startXPosition = null, currentXPosition = null, endXPosition = null;
    const // touch and event coordinates
        slides = getElementsByClassName(element, carouselItem), total = slides[length];
    let slideDirection = this[direction] = left;
    const leftArrow = getElementsByClassName(element, component + '-control-prev')[0],
    ...
    this.slideTo = function (next) {
        ...
        const activeItem = this.getActiveIndex();
        let // the current active
            orientation;
